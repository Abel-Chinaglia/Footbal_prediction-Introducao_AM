...
def gerar_dataset_1_resumido(data):
    ...
    for (club, year), grupo in data.groupby(['club_name', 'year']):
        ...
        pf = pf_valor[0] if len(pf_valor) == 1 else 2
        resultados.update({
...
def gerar_dataset_3_combinado(data_1, data_2, data_transfers, n_anos=1):
    """Gera o Dataset 3 combinando transferências e performance, com anos futuros intermediários apenas para n_anos=3 ou 5."""
    primeiro_ano = 2009
    ultimo_ano = 2019
    data_3 = data_2.copy()
    ...
    # Para n_anos=3 ou 5, adiciona transferências dos anos futuros intermediários
    if n_anos > 1:
        for anos_futuros in range(1, n_anos):
